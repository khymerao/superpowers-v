...
# The one signal that is the TOOL'S OWN VERDICT rather than a line of output:
# a non-zero exit. It survives on a read-only command's result; nothing else in
# ERROR_PATTERNS does.
EXIT_CODE_PATTERN = re.compile(r"exit code [1-9]")
...
# A heredoc opener: `<<WORD`, `<<-WORD`, `<<'WORD'`, `<<"WORD"`. `<<<` (a here
# string) deliberately does NOT match -- what follows `<<` there is `<`, which
# is none of the three alternatives.
_HEREDOC = re.compile(r"<<-?[ \t]*(?:'([^']*)'|\"([^\"]*)\"|([A-Za-z_][A-Za-z0-9_]*))")

# Commands that only READ. A traceback pasted by one of these is a quotation --
# the file it printed -- not this agent's own failure. See error_evidence.
READ_ONLY_COMMANDS = frozenset((
    "cat", "sed", "grep", "rg", "egrep", "fgrep", "head", "tail", "awk", "wc",
    "ls", "find", "diff", "file", "stat", "cut", "sort", "uniq", "tr", "nl",
    "column", "jq", "echo", "printf", "pwd", "which", "basename", "dirname",
    "realpath", "true", "false", "date", "env",
))
...
def strip_heredocs(command):
    """`command` with every heredoc BODY removed, openers left in place.

    A heredoc body is DATA, not command text: an agent writing a Python file
    with `cat > f.py <<PYEOF ... PYEOF` was reported as redirecting into `=`,
    because `assert s.count(old) >= 1` inside the body tokenized as a `>`
    followed by a word. Only the heredoc's own redirect target is a write.
    """
    ...
    while i < len(lines):
        ...
        for delim in [d for d in delims if d]:
...
def _is_write_target(token):
    """Whether a token after a redirect operator names a file.

    `true >= 1` tokenizes as `true`, `>`, `=`, `1`: shlex splits the comparison
    because `=` is not one of its punctuation characters. A `>=` is arithmetic,
    not a redirect into a file called `=`, so a target that opens with `=` -- or
    that is itself another operator -- is refused.
    """
    if not token or token in _SEPARATORS:
        return False
...
def is_read_only_command(command):
    """Whether every stage of `command` only reads.

    A `sed`/`cat`/`grep`/`head` that pastes a traceback has quoted a file; it
    has not failed. The check is deliberately strict -- one unrecognised
    command, one redirect, one unresolvable token and the answer is False --
    because suppressing a real failure costs more than reporting a quoted one.
    """
    ...
    for tok in tokens:
        if tok in _SEPARATORS:
            expect_command = True
            continue
        if tok.startswith(">") or (tok[:1].isdigit() and _REDIRECT.match(tok)):
            return False  # a redirect is a write, whatever runs in front of it
        if expect_command:
            if _unresolvable(tok) or os.path.basename(tok) not in READ_ONLY_COMMANDS:
                return False
            expect_command = False
    return not expect_command
    ...
    while i < len(tokens):
        ...
        if match and match.group(2):
            ...
            if rest.startswith("&"):  # 2>&1 — a descriptor, not a file
                ...
                continue
            if rest:
                if _is_write_target(rest):
                    targets.append(rest)
                i += 1
                continue
            if i + 1 < len(tokens):
                if _is_write_target(tokens[i + 1]):
...
def error_evidence(text, is_error=False, read_only=False):
    ...
    ANCHORED ONCE MORE on the command behind the result. When that command only
    reads -- `sed`, `cat`, `grep`, `head` -- its output is a QUOTATION of some
    file, so only the tool's own verdict counts: the harness's `is_error`, or a
    non-zero exit marker. A traceback such a command merely printed is not a
    failure of the agent that ran it, and this watcher reported several of them
    as one.
    """
    if not text:
        return None
    for pattern in ERROR_PATTERNS:
        if read_only and not is_error and pattern is not EXIT_CODE_PATTERN:
            continue
...
def _tool_name(entry):
    """The tool name from a tools-table entry. An entry is a bare name, or a
    [name, read_only] pair for a Bash call whose command only reads -- and a
    state file written by an older version carries only bare names."""
    if isinstance(entry, (list, tuple)):
        return entry[0] if entry else ""
    return entry


def _tool_read_only(entry):
    return bool(isinstance(entry, (list, tuple)) and len(entry) > 1 and entry[1])
...
def analyze_agent(path, agent_id, ctx, repo_root, is_allowed, start_line=0,
    ...
    for event in iter_tool_events(path, start_line):
        ...
        if kind == "tool_use":
            ...
            if name == "Bash" and is_read_only_command(command):
                tool_names[event.get("id")] = [name, 1]
            else:
                tool_names[event.get("id")] = name
            parsed = parse_register_lane(command)
...
def _selftest():  # noqa: C901 - a linear list of cases reads better than five helpers
    ...
    expect(">= is a comparison, not a redirect into a file called '='",
           bash_write_targets("true >= 1") == []
           and bash_write_targets("[ $n >= 2 ] && echo ok") == [])
    expect("a heredoc BODY is data: only the heredoc's own target is a write",
           bash_write_targets("cat > /tmp/x.py <<PYEOF\n"
                              "assert s.count(old) >= 1\n"
                              "print('a > b', '/etc/passwd')\n"
                              "PYEOF") == ["/tmp/x.py"])
    expect("...and a quoted heredoc delimiter closes the body just the same",
           bash_write_targets("python3 - <<'EOF'\nx = 1 >= 0\nEOF") == []
           and bash_write_targets("cat <<-EOF > out.txt\na > b\nEOF")
           == ["out.txt"])
    ...
    expect("sed/cat/grep/head only read",
           is_read_only_command("sed -n 1,20p f.py")
           and is_read_only_command("grep -n foo f.py | head -5")
           and is_read_only_command("cat a.txt"))
    expect("...but not once a redirect appears, nor for an unknown command",
           not is_read_only_command("cat a.txt > b.txt")
           and not is_read_only_command("grep -n foo f | tee log")
           and not is_read_only_command("python3 x.py")
           and not is_read_only_command("cat f && rm f"))
    ...
    expect("a read-only command that PASTES a traceback is not an error",
           error_evidence("Traceback (most recent call last):\n  File x",
                          False, True) is None
           and error_evidence("BLOCKED\ncommand not found", False, True) is None)
    expect("...but its own verdict still counts: is_error, or a non-zero exit",
           error_evidence("exit code 2", False, True) is not None
           and error_evidence("Traceback (most recent call last):", True, True)
           is not None)
