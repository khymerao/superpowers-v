...
```bash
"$CV/scripts/compound-v-run-cursor-worker.sh" \
...
way a job's `test_scope` could reach a headless Cursor worker was as a sentence inside
...
- It is **structurally validated before cursor-agent is launched**. A malformed contract, an unknown key,
