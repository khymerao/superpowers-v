...
7. **Continue the pipeline** from the reconciled phase: re-collect results, run the scope gate on every job, then the three-pass Review Gate (AC-gated), then merge worktree diffs on PASS. Already-`done` jobs are not re-run. **On reaching `MERGED`, do not write the phase by hand** — the workflow finalizer advanced it and committed the run directory (3.4.1); read it back, commit only the memory stream this step appended, exactly as [`parallel-dispatcher`](../agents/parallel-dispatcher.md)'s Step 7 now does.
